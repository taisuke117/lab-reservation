...
def show_calendar_page(title, equipment_colors, page_key):
    ...
    st.caption("📌 空き時間をクリック → 新規予約　／　既存の予約をクリック → 詳細・修正・削除")
    if st.button("🏠 トップに戻る"):
        ...
        def show_new_reservation_dialog(init_start, init_end):
            ...
            with col1:
                ...
                start_hour = st.selectbox(
                    "開始時間", list(range(25)), index=init_start.hour,
                    format_func=lambda h: f"{h:02d}:00"
                )
            with col2:
                end_date = st.date_input("終了日", init_end.date())
                end_hour = st.selectbox(
                    "終了時間", list(range(25)), index=min(init_end.hour, 24),
                    format_func=lambda h: f"{h:02d}:00"
                )
    ...
    if cal_result and cal_result.get("eventClick"):
        ...
        if not df_click.empty:
            ...
            def show_reservation_dialog(row):
                mode = st.radio("モード", ["詳細", "修正"], horizontal=True)
                ...
                if mode == "詳細":
                    ...
                    nick_index = user_options.index(current_nick) if current_nick in user_options else 0
                    nickname = st.selectbox("利用者", user_options, index=nick_index)

                    eq_index = equipment_list.index(row['equipment']) if row['equipment'] in equipment_list else 0
                    equipment = st.selectbox("機器を選択", equipment_list, index=eq_index)

                    try:
                        cur_start = datetime.fromisoformat(str(row['start_datetime'])[:16])
                        cur_end = datetime.fromisoformat(str(row['end_datetime'])[:16])
                    except Exception:
                        cur_start = datetime.now().replace(minute=0, second=0, microsecond=0)
                        cur_end = cur_start + timedelta(hours=1)

                    col1, col2 = st.columns(2)
                    with col1:
                        start_date = st.date_input("開始日", cur_start.date())
                        start_hour = st.selectbox(
                            "開始時間", list(range(25)), index=cur_start.hour,
                            format_func=lambda h: f"{h:02d}:00", key="edit_start"
                        )
                    with col2:
                        end_date = st.date_input("終了日", cur_end.date())
                        end_hour = st.selectbox(
                            "終了時間", list(range(25)), index=min(cur_end.hour, 24),
                            format_func=lambda h: f"{h:02d}:00", key="edit_end"
                        )

                    current_notes = str(row.get('notes', '')).strip()
                    if current_notes == "nan":
                        current_notes = ""
                    notes = st.text_area("備考（任意）", value=current_notes, height=80)
                        ...
                        if st.button("💾 修正を保存", type="primary", use_container_width=True):
                            if nickname == "名前を選択してください":
                                ...
                                if start_dt >= end_dt:
                                    ...
                                    if check_conflict(equipment, start_dt, end_dt, exclude_id=int(row["id"])):
                                        st.error("⚠️ その時間は既に別の予約が入っています。")
                                    else:
                                        update_reservation(int(row["id"]), nickname, equipment, start_dt, end_dt, notes)
                                        st.success("修正しました！")
                                        st.rerun()
