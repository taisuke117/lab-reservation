# lab-reservation
# 🔬 NDUP機器 予約システム <!--VERSION-->v2.260828<!--/VERSION-->
病理学講座の機器予約管理システムです。カレンダーをクリックするだけで予約・修正・削除ができます。
---
## 🌐 アクセスURL
管理者に問い合わせてください。
---
## 📖 使い方
### 予約する
1. トップページからカテゴリを選ぶ
2. カレンダーの空き時間をクリック（またはドラッグ）
3. 利用者・機器・日時を確認して「✅ 予約する」を押す
4. 備考欄は任意入力です。記入した内容はカレンダー上にも表示されます
### 修正する
1. カレンダー上の予約をクリック
2. 詳細ダイアログで「修正」を選ぶ
3. 内容を変更して「💾 修正を保存」を押す
### 削除する
1. カレンダー上の予約をクリック
2. 詳細ダイアログで「本当に削除してよいですか？」にチェック
3. 「🗑️ 削除する」を押す
---
## 🗂 カテゴリ一覧
| カテゴリ | 機器 |
|---|---|
| 🔪 ミクロトーム | 回転式ミクロトーム（A）:旧 ／回転式ミクロトーム（B）:新 ／滑走型ミクロトーム |
| 🎨 実験台（実験室） | 実験台１／実験台２／実験台３ (廊下側から窓側に向かって1, 2, 3) ※分生エリアの机除く|
| 📦 包埋ロータリー・包埋センター | 包埋ロータリー／包埋センター |
| 🎥 バーチャル撮影装置 | バーチャル撮影装置 |
| 🧬 分生関係 | 分生エリア／分生室 |
| 🧫 培養室 | 安全キャビネット／クリーンベンチ |

各カテゴリには上記の機器に加えて「その他の作業1」「その他の作業2」があります。特定の機器の予約ではなく、何か予約表に情報として残したいときに使ってください（枠は2つあるので、同じ時間帯に2件まで登録できます）。
---
## 🏗 システム構成
```
利用者のブラウザ
　　↕ URL接続
Streamlit Cloud（アプリサーバー）
　　↕ データ読み書き
Supabase（クラウドDB）

GitHub Actions（定期ping）→ Streamlit Cloud をスリープさせない
```
### 使用技術
| 技術 | 役割 |
|---|---|
| Python / Streamlit | Webアプリの画面 |
| streamlit-calendar | カレンダー表示（FullCalendar） |
| Supabase (PostgreSQL) | 予約データの永続保存 |
| Streamlit Cloud | 24時間稼働サーバー |
| GitHub | コード管理 |
| GitHub Actions | アプリの自動稼働維持（スリープ防止） |
---
Streamlit   https://share.streamlit.io/  
Supabase   https://supabase.com/
## 📁 ファイル構成
```
lab-reservation/
├── Home.py                          ← トップページ
├── utils.py                         ← 共通処理（DB・カレンダー・ダイアログ・認証）
├── app_version.py                   ← バージョン情報（唯一の定義場所）
├── requirements.txt                 ← 依存ライブラリ
├── README.md                        ← このファイル
├── tools/
│   └── update_version.py            ← バージョンを README へ同期するスクリプト
├── .github/
│   └── workflows/
│       └── keep-awake.yml           ← アプリのスリープ防止（定期ping）
└── pages/
    ├── 1_ミクロトーム.py
    ├── 2_実験台（実験室）.py
    ├── 3_包埋ロータリー・包埋センター.py
    ├── 4_バーチャル撮影装置.py
    ├── 5_分生関係.py
    ├── 6_培養室.py
    └── 7_全体カレンダー.py
```
---
## 🔒 セキュリティ
機密情報（APIキー、パスワード等）はすべてStreamlit CloudのSecretsに暗号化保存しており、GitHubには一切アップロードしていません。
### Secretsの構成（管理者用）
```toml
USERS = '["氏名1", "氏名2", ...]'
SUPABASE_URL = "https://xxx.supabase.co"
SUPABASE_KEY = "sb_secret_xxx"
app_password = "閲覧用の共通パスワード"
```
### パスワードの自動入力について
ログイン画面のパスワード欄は `autocomplete="current-password"` を指定してあり、ブラウザのパスワードマネージャから自動入力できます。

ただし Streamlit はフォーム送信もページ遷移も行わないため、**ブラウザが「パスワードを保存しますか？」と聞いてくることはありません**。初回だけ手動で登録してください。

Chrome の場合は `chrome://password-manager/passwords` →「追加」で、サイトにアプリの URL、ユーザー名（任意・空欄可）、パスワードを入力します。以降は自動入力が効きます。

---
## 🏷 バージョン
表記は `v<メジャー>.<YYMMDD>`（例: `v2.260725` = メジャー2・2026-07-25 更新）。

**バージョンの定義場所は `app_version.py` の1か所だけ**です。アプリ画面のフッターはここから読んでいるので、`.py` を個別に書き換える必要はありません。

README は Markdown で import できないため、同期用のスクリプトを用意しています。リポジトリ直下で実行してください。

```bash
python tools/update_version.py             # 当日の日付でバージョンを上げる
python tools/update_version.py v2.260801   # バージョンを明示指定する
python tools/update_version.py --check     # 書き換えずにズレの有無だけ確認する
```

README 側は、このファイル冒頭の見出し行にあるバージョン部分だけが書き換わります。HTML コメントのマーカーで囲んであり、表示はされません。**このマーカーは消さないでください**（消すとスクリプトがエラーで止まります）。

---
## ♻️ 自動稼働維持（スリープ防止）
Streamlit Community Cloud のアプリは、アクセスがない状態が続くと自動的にスリープし、次回アクセス時に起動待ち（数十秒〜数分）が発生します。
これを防ぐため、`.github/workflows/keep-awake.yml` で GitHub Actions が定期的にアプリへ ping を送り、起きた状態を維持しています。

- 実行間隔: 6時間おき（`cron` で設定。GitHub側の都合で多少前後します）
- 送信先: アプリのヘルスチェック用エンドポイント `/_stcore/health`
- アプリURLは GitHub Secrets の `APP_URL` に登録（リポジトリ上には記載しない）

### GitHub Secrets の構成（管理者用）
`Settings` →「Secrets and variables」→「Actions」で登録します。

| Name | 値 |
|---|---|
| `APP_URL` | `https://xxx.streamlit.app/` |

### 手動実行・動作確認
`Actions` タブ →「Keep Streamlit app awake」→「Run workflow」で手動実行できます。
ログに `HTTP 200`（またはリダイレクトを示す `3xx`）が出れば、サーバーへの通信は成功しています。
---
## 🛠 メンバー追加・変更方法
Streamlit Cloudの「Settings」→「Secrets」で`USERS`の配列を編集し、「Save」→「Reboot app」で反映されます。
実施は管理者が行います。
---
## 📞 管理者
Taisuke Hani
